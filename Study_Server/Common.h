...
void err_quit(const char* msg)
{
	...
	FormatMessageA( //A = Ascii
		...
		nullptr/* 어디서 찾을지 지정필요 X*/, WSAGetLastError(),
		MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT),
		// MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT) 윈도우 운영체제 기본 언어로 오류문자 가져옴.
		(char*)&lpMsgBuf/*버퍼의 시작 메모리*/, 0/*크기도 자동 지정*/, nullptr/*아규먼트도 자동 지정*/);
	//lpMsgBuf메모리 넘기기
